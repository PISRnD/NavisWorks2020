...
namespace ClashClusters
{
    ...
    {
        public static void GroupClashes(ClashTest selectedClashTest, GroupingMode groupingMode, GroupingMode subgroupingMode, bool keepExistingGroups, bool IsOtherOption)
        {
            ...
            if (IsOtherOption == true)
            {
                clashResultGroups = GroupElementWiseClashCount(clashResults, "");
            }
            else
            {
        ...
        private static List<ClashResultGroup> GroupElementWiseClashCount(List<ClashResult> results, string initialName)
        {
            ...
            List<ModelItem> ModelList1 = new List<ModelItem>();
            List<ModelItem> ModelList2 = new List<ModelItem>();
            List<ClashItemInfo> info = new List<ClashItemInfo>();
            int count = 0;
            foreach (ClashResult result in results)
            {
                //Cannot add original result to new clash test, so I create a copy
                ClashResult copiedResult = (ClashResult)result.CreateCopy();
                ClashItemInfo cii = new ClashItemInfo();
                cii._modelItem1Code = copiedResult.Item1.InstanceHashCode;
                cii._modelItem2Code = copiedResult.Item2.InstanceHashCode;
                cii._modelItem1 = copiedResult.Item1;
                cii._modelItem2 = copiedResult.Item2;
                cii._clashName = copiedResult.DisplayName;
                cii._clashresult = result;
                info.Add(cii);
            }
            var orderGroups = info.GroupBy(w => w._modelItem1Code);
            var orderGroups1 = info.GroupBy(w => w, new ListForUniQgroup());
            int GrpCount = orderGroups.Count();
            int GrpCount1 = orderGroups.Count();
            ////if (copiedResult.CompositeItem1 != null)
            ////{
            //ModelList1.Add(copiedResult.Item1);
            //    //modelItem = GetSignificantAncestorOrSelf(copiedResult.CompositeItem1);
            ////}
            ////else if (copiedResult.CompositeItem2 != null)
            ////{
            //ModelList2.Add(copiedResult.Item2);
    ...
    public class ClashItemInfo
    {
        public ModelItem _modelItem1 { get; set; }
        public ModelItem _modelItem2 { get; set; }
        public ClashResult _clashresult { get; set; }
        public int _modelItem1Code { get; set; }
        public int _modelItem2Code { get; set; }
        public string _clashName { get; set; }
    }
    class ListForUniQgroup : IEqualityComparer<ClashItemInfo>
    {
        bool isItem2 = false;
        public bool Equals(ClashItemInfo a, ClashItemInfo b)
        {
            if (a._modelItem1Code == b._modelItem1Code || a._modelItem1Code == b._modelItem2Code)
            {
                //return a == b;
                return true;
            }
            else if (a._modelItem2Code == b._modelItem2Code)
            {
                isItem2 = true;
                return true;
            }
            ...
            //return a.MyMtLocation.Equals(b.MyMtLocation);
        }
        public int GetHashCode(ClashItemInfo obj)
        {
            //return obj._clashresult.GetHashCode();
            if (isItem2 == true)
            {
                return obj._modelItem2Code;
            }
            return obj._modelItem1Code;
        }
    }
