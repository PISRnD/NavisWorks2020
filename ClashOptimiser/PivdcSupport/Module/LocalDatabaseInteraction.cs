            ...
            if (File.Exists(localDBLocation))
            {
                try
                {
                    ...
                    try
                    {
                        sQLiteConnection.Open();
                        SQLiteCommand sQLiteCommand = sQLiteConnection.CreateCommand();
                        string sqlQuery = string.Format("SELECT EMP_ID,EMP_NAME FROM Local_Login where  DomainName ='{0}'", SupportDatas.CurrentLoginInformation.DomainName);
                        ...
                        sQLiteConnection.Close();
                        string imageLocation = string.Format("{0}\\Images", SupportDatas.AssemblyDirectory);
