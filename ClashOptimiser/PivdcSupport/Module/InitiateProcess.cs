    ...
    public static class InitiateProcess
    {
        public static bool Initialize(string assmeblyFullName, string softwareVersion, bool isCommand)
        {
            ...
            if (!isCommand)
            {
                ...
                {
                    TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The Ribbon assembly is not found.\nContact your provider for assistance.");
                    return false;
                }
            }
            ...
            {
                TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The configuration file is missing.\nContact your provider for assistance.");
                return false;
            }
            tempData = string.Format("{0}\\AccessType.ptlt", SupportDatas.AssemblyDirectory);
            if (!File.Exists(tempData))
            {
                TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The License type verification file does not exists.\nContact your provider for assistance.");
                return false;
            }
            ...
            {
                TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The License type verification file is corrupted.\nContact your provider for assistance.");
                return false;
            }
            ...
            if (!File.Exists(tempData))
            {
                TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The connection option file does not exists.\nContact your provider for assistance.");
                return false;
            }
            tempData = File.ReadAllText(tempData);
            if (string.IsNullOrEmpty(tempData) || string.IsNullOrWhiteSpace(tempData) || !tempData.Contains("PISD") || !tempData.Contains("local-road"))
            {
                TaskDialog.Show(SupportDatas.TaskDialogErrorTitle, "The connection option file is corrupted.\nContact your provider for assistance.");
                return false;
            }
