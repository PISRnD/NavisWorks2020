...
namespace PivdcNavisworksSupportModule
{
    public static class ToolSupport
    {
        ...
        public static bool IsValidTool(this Document document, string toolDatabaseName, bool callingFromRibbon)
        {
            ...
            if (!validTool)
            {
                ShowErrorMessageNavis(SupportDatas.TaskDialogErrorTitle, "You are not in connection, contact your provider for assistance");
            }
            return validTool;
        }
        ...
        public static bool OpenLoginProjectInfoWindow(this Document document, string toolNameInDatabase)
        {
            ...
            {
                /* For Revit Only* isProjctIdValidate = ProjectIdValidation.IsProjectInformationValidate(document, SupportDatas.RevitToolConnectionString); */
                ...
                if (SupportDatas.ProjectCode == "Unknown")
                {
                    ...
                    if (projectId > 0)
                    {
                        ...
                        using (Transaction transaction = new Transaction(document, "WriteProjectInformation"))
                        {
                            try
                            {
                                /* For Revit Only* transaction.Start();
                            ...
                            {
                                /*For Revit Only if (transaction.GetStatus() == TransactionStatus.Started) transaction.RollBack(); */
                        ...
                        /*For Revit Only* SupportDatas.ProjectId = ProjectInformationReadWrite.ReadProjectInformation(document); */
                    }
        ...
        public static void InsertUsage(this Document document, long numberOfProcessedElement, [Optional] long timeTaken)
        {
            if (SupportDatas.InsertUsageVaiAmazon)
            {
                ...
                {
                    addin_id = Convert.ToString(SupportDatas.ToolIdPerDatabase),
                    addin_name = SupportDatas.ToolNameInDatabase,
                    addin_ver = SupportDatas.RevitVersion,
                    dtp_name = Environment.MachineName,
                    project = Convert.ToString(SupportDatas.ProjectId),
                    time_taken_ms = Convert.ToString(timeTaken),
                    time_taken = Convert.ToString((timeTaken / 1000)),
                    domain_name = System.Security.Principal.WindowsIdentity.GetCurrent().Name,
                    emp_id = SupportDatas.CurrentLoginInformation.EmployeeId,
                    filename = document.FileName,
                    ip = OnlineLicenseCheck.GetLocalIPAddress(),
                    mac = OnlineLicenseCheck.GetMacAddress(),
                    no_element_proccess = Convert.ToString(numberOfProcessedElement)
                };
                ...
                if (!usage.InsertUsage(usageDetail))
                {
            ...
            else
            {
                if (SupportDatas.ToolLicenseAccessType == "000int")
                {
                    ...
                    if (!DatabaseManager.InsertUsages(SupportDatas.ToolNameInDatabase, SupportDatas.ToolIdPerDatabase, Environment.MachineName,
                         ...
                         SupportDatas.RevitVersion, document.FileName, false))
                    {
                        new Exception("Unable to Insert the tool usage detail").ErrorLogEntry(MethodBase.GetCurrentMethod().Name);
                    }
                }
                ...
                {
                    ShowErrorMessageNavis("PiVDC tool says : Error message", "The plugin origin is from Pinnacle Infotech");
                }
            }
        }
        ...
        {
#if false
            /* For Revit file path Implemented by Umakanta Mondal */
        ...
        public static void ShowErrorMessageNavis(string windowTitle, string errorMessage)
        {
            Autodesk.Windows.TaskDialog taskDialog = new Autodesk.Windows.TaskDialog
            {
        ...
        {
            ShowErrorMessageNavis(SupportDatas.TaskDialogErrorTitle, exception.Message);
        }

        public static void ShowErrorMessage(this string errorMessage, string errorMessageFileName)
        {
            ShowErrorMessageNavis(SupportDatas.TaskDialogErrorTitle, errorMessageFileName);
