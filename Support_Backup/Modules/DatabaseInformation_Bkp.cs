...
namespace PivdcNavisworksSupportModule
{
    ...
    /// </summary>
    public static class DatabaseInformation_Bkp
    {
        ...
        public static int IsValidRevitTool(string addinName, string connectionString, bool offLine = false)
        {
            int count = 0;
            SupportDatas.ToolNameInDatabase = addinName;
            if (!offLine)
            {
                SqlConnection sqlConnection = new SqlConnection(connectionString);
                try
        ...
        public static string EmployeeIdByLoggedIn(string localDBLocation, string localDBConnectionString, string centralLoginConnectionString, bool isOffline = false)
        {
            ...
            if (System.IO.File.Exists(localDBLocation))
            {
                try
                {
                    ...
                    if (string.IsNullOrEmpty(SupportDatas.CurrentLoginInformation.EmployeeId) || !SupportDatas.CurrentLoginInformation.LoginStatus)
                    {
                        ...
                        if (SupportDatas.CurrentLoginInformation.LoginStatus)
                        {
                            employeeId = SupportDatas.CurrentLoginInformation.EmployeeId;
                        }
        ...
        public static int UATToolStatus(string connectionString, string projectInfromationConnectionString, string toolName, bool isRevitTool, bool isOffline = false)
        {
            //0=Offline tool
            //1=Under UAT
            //2=Over UAT
            ...
            try
            {
                ...
                if (isRevitTool)
                {
                    isValidToolId = IsValidRevitTool(toolName, connectionString, isOffline);
                }
                else
                {
                    isValidToolId = IsValidAutoCADTool(toolName, connectionString, isOffline);
                }
                if (isValidToolId > 0)
                {
                    /*SupportDatas.CurrentLoginInformation = LocalDatabaseInteraction
                        .HasLoggedInformation(SupportDatas.LocalDBLocation, SupportDatas.LocalDBConnectionString);*/
                    if (!string.IsNullOrEmpty(SupportDatas.CurrentLoginInformation.EmployeeId))
                    {
        ...
        public static int GetProjectIdNCode(string projectInfromationConnectionString, out string projectCode)
        {
           ...
           /* PISProjectInformationUI pisProjectInformationUI = new PISProjectInformationUI(projectInfromationConnectionString);
            pisProjectInformationUI.ShowDialog();*/
            if (SupportDatas.ProjectId > 0)
            {
                projectId = SupportDatas.ProjectId;
            }
            projCode = SupportDatas.ProjectCode;
